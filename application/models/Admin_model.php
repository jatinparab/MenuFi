...
class Admin_model Extends CI_Model {
	...
	public function removeItemF($id){
		$query = "DELETE FROM fake_order WHERE id ='".$id."'";
		return $this->db->query($query);
	}
	public function editItemF($id,$value){
		$query = "UPDATE fake_order SET Quantity = '".$value."' WHERE id ='".$id."'";
		return $this->db->query($query);
	}
    ...
    public function clearFake(){
    	$this->db->query("DELETE FROM `fake_order` WHERE 1");
...
public function addInventoryToDb($d){
    ...
    $data['inventorycategory']=$d['inventorycategory'];
    $data['addons']=$d['addons'];
    $data['cost']=$d['cost'];
    $this->db->insert('ingredients',$data);
    $last_id = $this->db->insert_id();
    if($last_id>0){
        echo '<script>alert("Inventory Category Added");'
        . 'window.location.href="'.  base_url().'index.php/Admin/inventory"</script>';
    }
}

public function addInventoryCategoryToDb($d){
   	$data['Name']=$d['Name'];
...
public function updateInventoryCategoryToDb($d){
    $data['name']=$d['Name'];
    
    $this->db->where('id',$this->input->post('inv_id'));
    
    $status = $this->db->update('inventorycategory',$data);
...
//                                $this->db->order_by('co.Order_id', 'DESC');
		...
		public function get_today_orders($date){
			...
			return $this->db->query("SELECT * FROM orders WHERE DATE(Timestamp) = '$date'")->result_array();

		}

		public function printOrderDetails($Order_id){
        
			//$Order_id='520';
		   
			$printData = $this->get_print_details($Order_id);
			...
			$table= "<table class='col-sm-3 col-sm-offset-1' style='font-size: 15px;'><tr><td colspan='4' align='center' style='font-weight:bold;font-size: 24px;'></td></tr>
			  <tr><td colspan='4' style='border-bottom-style: dotted;border-width: 1px;'></td></tr>
				<tr>  
				  <td>Order No : ".$Order_id."</td>   <td style='text-align:right' colspan='2'>Time : ".date('G:i A',strtotime($sq4['0']['Timestamp']))."</td>
				</tr>
				<tr>  
				  <td>Type : ".$printData[0]['order_type']."</td>   <td style='text-align:right' colspan='2'>Time : ".date('d-m-Y',strtotime($sq4['0']['Timestamp']))."</td>
				</tr>
			 ...
			 $ds= $this->db->query("SELECT * FROM payment_details WHERE Order_id='$Order_id'");
			  ...
			  $table .="
			   ...
			   <td></td><td style='font-weight:bold;text-align:right'>Sub Total</td><td style='text-align:right'> ".ceil($dc)."</td>
				...
				<tr>
				  <td></td><td style='font-weight:bold;text-align:right'>Total Amount</td><td style='text-align:right'>".$res['total_amount']."</td>
				</tr>
				<tr><td colspan='4' style='border-bottom-style: dotted;border-width: 1px;'></td></tr>
				<tr></div>";
		...
		public function completeOrder($d)
		{
			...
			/*     echo "|".$mid[$i];
					...
					$login_type = 'Manual';
					$k = $addon[$i];
					$md = (empty($batter[$i]))?0:$batter[$i];
					// echo $k;
					if($qty == 0){
						$sc = $this->db->query("DELETE FROM `fake_order` WHERE Menu_id = $mid[$i] AND Customer_id = $cid");
					}
					else{
						$x = $this->db->query("SELECT * FROM `customer_order` WHERE Menu_id= $mid[$i] AND Customer_id=$cid") -> result_array();
						if (count($x)>0){
							$sc = $this->db->query("DELETE FROM `fake_order` WHERE Menu_id = $mid[$i] AND Customer_id = $cid");				
							$success = $this->db->query("UPDATE customer_order SET `Quantity` = `Quantity` + 1 WHERE Customer_id = $cid AND Menu_id = $mid[$i]");
						}else{
						$sc = $this->db->query("DELETE FROM `fake_order` WHERE Menu_id = $mid[$i] AND Customer_id = $cid");				
       ...
       //                          $addons = explode(',', $raw['Addons']);
                                ...
                                {
                                    $sql_addons = $this->db->query("SELECT * FROM ingredients WHERE Ingredients_id='".trim($va)."'");
                                    $ra = $sql_addons->result_array();
                                    $rs = $ra[0];
                                    $addons_price += (int)$item->Quantity*$rs['cost'];
                                }
                            $total += (float)$addons_price;
                            // }
						}
						$cgst = ($total/100)*2.5;
						$sgst = ($total/100)*2.5;
						$net_total = ceil($total + $cgst + $sgst);
						$data = array(
								'cgst' => $cgst,
								'sgst' => $sgst,
								'net_total' => $net_total,	
       ...
       //                          $addons = explode(',', $raw['Addons']);
                                ...
                                foreach ($addons as $key => $va)
                                {
                                    ...
                                    $ra = $sql_addons->result_array();
                                    $rs = $ra[0];
                                    $addons_price += $rs['cost']*(int)$item->Quantity;
                                }
                            $total += (float)$addons_price;
						}
						$cgst = ($total/100)*2.5;
...
halfYear")->result();
		...
		public function get_print_details($Order_id){
			...
			$this->db->select('co.Quantity,co.Addons,co.Menu_Id,m.Name,m.Price,s.cgst,s.sgst,s.net_total,s.coupon_apply,c.c_code,c.c_type,c.c_value,o.order_type');
			$this->db->from('customer_order co');
			$this->db->join('menu m', 'co.Menu_id = m.Menu_id');
